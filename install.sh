...
echo
echo "************************"
echo "Spotify-Mac by @ALBINPRAVEEN"
echo "************************"
echo

# Report versions
echo -e "Spotify version: ${CLIENT_VERSION}"
echo -e "Spotify-Mac version: ${spotify_VERSION}\n"
...
if [[ ! -f "${XPUI_SPA}" ]]; then
  ...
  if [[ "${FORCE_FLAG}" == "false" ]]; then
    if [[ -f "${XPUI_BAK}" ]]; then
      echo "spotify backup found, spotify has already been used on this install."
      echo -e "Re-run spotify using the '-f' flag to force xpui patching.\n"
      echo "Skipping xpui patches and continuing spotify..."
      XPUI_SKIP="true"
    else
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  ...
  if grep -Fq "spotify" "${XPUI_JS}"; then
    echo -e "\nWarning: Detected spotify patches but no backup file!"
...
echo -e "spotify finished patching!\n"